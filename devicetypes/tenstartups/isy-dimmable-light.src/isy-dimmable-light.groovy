...
metadata {
  definition (name: "ISY Dimmable Light", author: "marclennox", namespace: "TenStartups") {
    capability "Switch"
...
def on() {
  log.debug "Turning dimmable light ON..."
  sendCommand("on")
}

def off() {
  log.debug "Turning dimmable light OFF..."
  sendCommand("off")
}

def setLevel(level) {
  log.debug "Setting dimmable light level to ${level}%..."
...
def sendCommand(String commandPath) {
  new physicalgraph.device.HubAction(
    [
       method: "POST",
       path: "/api/dimmable_light/${getDataValue("externalId")}/${commandPath}",
...
def processStatusUpdate(data) {
  if (data.status != null) {
	  log.debug "Dimmable light status is ${data.status.toUpperCase()}"
	  sendEvent(name: "switch", value: data.status)
  }
  if (data.level != null) {
	  log.debug "Dimmable light level is ${data.level}%"
